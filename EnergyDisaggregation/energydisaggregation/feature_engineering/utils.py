...
from EnergyDisaggregation.energydisaggregation.feature_engineering.temperature import (
    get_critval,
    temperature_ressentie,
    temperature_lagh,
    nebul_features,
)
from EnergyDisaggregation.energydisaggregation.feature_engineering.cyclical_transformation import (
    cyclical_encoder,
)
from EnergyDisaggregation.energydisaggregation.feature_engineering.calendar import (
    get_calendar_features,
)
from EnergyDisaggregation.energydisaggregation.feature_engineering.historical import (
    timeseries_dataframe,
)
from EnergyDisaggregation.energydisaggregation.dataloader.config import (
    DATACONFIG,
    CONFIG_WEATHER,
    CONFIG_POWER,
)
...
df_tot = Dataloader.load_data(
    path_power=raw_power_filename,
    path_weather=raw_weather_filename,
    frequency="h",
    resampling_mode_power="sum",
    resampling_mode_weather="mean",
)
...
def get_dataframe(temp_lagh=48, neb_lagh=3, time_lagh=7):
    ...
    df_cycl_h = cyclical_encoder(df_calendar, "hour", 24)
